function varargout = TrapezGUI(varargin)

gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @TrapezGUI_OpeningFcn, ...
                   'gui_OutputFcn',  @TrapezGUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function TrapezGUI_OpeningFcn(hObject, eventdata, handles, varargin)

handles.output = hObject;

guidata(hObject, handles);

function varargout = TrapezGUI_OutputFcn(hObject, eventdata, handles) 
...
x=0:0.001:3*pi;
plot(handles.axes,x,f(x));
hold on
plot(handles.axes,x,g(x));
legend('f(x)','g(x)');
xlabel('x');
ylabel('f(x),g(x)')
...
vf=Trap(f,startValue,finishValue,n);
vg=Trap(g,startValue,finishValue,n);
